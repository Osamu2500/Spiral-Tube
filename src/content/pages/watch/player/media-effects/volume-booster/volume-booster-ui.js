  ...
  static toggleEQPanel(ctx, video, anchorBtn) {
    ...
    if (ctx._volumePopup) {
      ...
      if (ctx._volumePopupOutsideHandler) {
        if (ctx.removeListener)
          ctx.removeListener(document, 'pointerdown', ctx._volumePopupOutsideHandler);
        document.removeEventListener('pointerdown', ctx._volumePopupOutsideHandler);
            ...
            if (chrome?.storage?.local) {
              chrome.storage.local
                ...
                .then((data) => {
                  const updated = {
                    ...(data.settings || {}),
                    volumeCustomPresets: JSON.stringify(custom),
                  };
                  chrome.storage.local.set({ settings: updated });
                })
                .catch(() => {});
    ...
    addPresetBtn.onclick = () => {
      ...
      const newPreset = {
        eq: [...ctx._eqGains],
        compressor:
          ctx._compressorEnabled && ctx.compressorNode
            ? {
                ratio: ctx.compressorNode.ratio.value,
                threshold: ctx.compressorNode.threshold.value,
              }
            : null,
    ...
    const mkDynRow = (label, min, max, step, val, unit, onChange) => {
      ...
      lbl.style.cssText =
        'font-size:10px;font-weight:700;color:rgba(255,255,255,0.45);text-transform:uppercase;letter-spacing:0.5px;min-width:80px;';
    ...
    resetBtn.onclick = () => {
      ...
      if (sRow) {
        ...
        if (ranges[0]) {
          ranges[0].value = 100;
          ranges[0].nextElementSibling.textContent = '100%';
        }
        if (ranges[1]) {
          ranges[1].value = 0;
          ranges[1].nextElementSibling.textContent = '0%';
        }
        if (ranges[2]) {
          ranges[2].value = 1.0;
          ranges[2].nextElementSibling.textContent = '1x';
        }
        if (ranges[3]) {
          ranges[3].value = 0;
          ranges[3].nextElementSibling.textContent = '0%';
        }
