...
export const AudioDynamicsMixin = {
    ...
    setAutoGain(enabled) {
        ...
        if (this.ctx && this.ctx.state === 'suspended') {
            this.ctx.resume().catch((e) => {
                this.utils?.log?.('[YPP:Equaliser] setAutoGain resume failed: ' + e.message, 'Equaliser', 'info');
            });
        }
    ...
    setWarmth(amount, bypassOnly = false) {
        if (!bypassOnly) this._warmthAmount = amount;
        if (this._proxyCmd('setWarmth', amount)) return;
        if (!this._audioConnected && this._needsAudioGraph && this._needsAudioGraph()) {
            const video = this._boundVideo || window.YPP.DOMManager?.getVideo();
            if (video) this.initAudioContext(video);
        }
        if (this.warmthNode) {
            if (amount <= 0) {
                // Basically bypass the shaping by using a linear curve
                this.warmthNode.curve = new Float32Array([ -1, 1 ]);
                if (this.warmthLpNode) {
                    this.warmthLpNode.frequency.value = 24000;
                }
            } else {
                // Use asymmetrical tube saturation
                if (this._makeTubeCurve) {
                    this.warmthNode.curve = this._makeTubeCurve(amount);
                }
                // Roll off high frequencies generated by saturation
                if (this.warmthLpNode) {
                    // Map 0-100% warmth to 15kHz -> 8kHz lowpass
                    const freq = 15000 - (amount / 100) * 7000;
                    this.warmthLpNode.frequency.setTargetAtTime(freq, this.ctx ? this.ctx.currentTime : 0, 0.1);
                }
            }
        }
    }
