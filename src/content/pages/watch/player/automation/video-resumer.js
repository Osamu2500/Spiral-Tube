...
export class VideoResumer extends window.YPP.features.BaseFeature {
    ...
    async restoreTime() {
        ...
        try {
            ...
            if (Math.abs(this.videoElement.currentTime - savedTime) > 2 && savedTime > VideoResumer.CONFIG.MIN_RESUME_SECONDS) {
                ...
                if (videoDuration && savedTime / videoDuration > VideoResumer.CONFIG.COMPLETION_THRESHOLD) {
                    if (chrome && chrome.storage && chrome.storage.sync) {
    ...
    _calculateResumeTime(savedTime) {
        ...
        let resumeTime = Math.max(0, savedTime - VideoResumer.CONFIG.CONTEXT_REWIND_SECONDS); // Default context rewind

        for (let i = sorted.length - 1; i >= 0; i--) {
            const chap = sorted[i];
            if (chap <= savedTime) {
                // If we are within threshold of a chapter start, snap to it
                if (savedTime - chap <= VideoResumer.CONFIG.CHAPTER_SNAP_SECONDS) {
                    resumeTime = chap;
                }
                break;
            }
    ...
    forceSave() {
        ...
        try {
            if (duration && (currentTime / duration > VideoResumer.CONFIG.COMPLETION_THRESHOLD)) {
                if (chrome && chrome.storage && chrome.storage.sync) {
                    chrome.storage.sync.remove(key);
                } else {
                    window.YPP.StorageManager.remove(key);
    ...
    async _pruneOldResumes() {
        try {
            ...
            if (keys.length <= VideoResumer.CONFIG.MAX_RESUME_HISTORY) return;
            ...
            const toDelete = parsed.slice(0, parsed.length - VideoResumer.CONFIG.MAX_RESUME_HISTORY).map(item => item.key);
    ...
    handleTimeUpdate() {
        ...
        const now = Date.now();
        // Relaxed interval for performance, relying on pagehide for accuracy
        if (!this.lastSave || now - this.lastSave > VideoResumer.CONFIG.SAVE_THROTTLE_MS) {
            this.forceSave();
            this.lastSave = now;
        }
    }
