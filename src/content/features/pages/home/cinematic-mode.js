...
window.YPP.features.CinematicMode = class CinematicMode extends window.YPP.features.BaseFeature {
    ...
    async _simulateHover(element) {
        ...
        // Block mouse leave events on both the element and thumbnail to prevent native cleanup
        ...
        // Start monitoring for the native preview element
        this._monitorNativePreview(card);

        // Add an extra delay to allow native preview to load
        await new Promise(r => setTimeout(r, 1000));
    ...
    _monitorNativePreview(card) {
        ...
        this._previewObserver = new MutationObserver(() => {
            const preview = card.querySelector('ytd-video-preview');
            if (preview && !preview.hasAttribute('data-ypp-hero-projected')) {
                this._projectNativePreview(card, preview);
            }
        });
        
        this._previewObserver.observe(card, { childList: true, subtree: true });
    }

    _projectNativePreview(card, preview) {
        if (!this._heroState.heroElement) return;
        
        // Disconnect the observer since we found the preview
        if (this._previewObserver) {
            this._previewObserver.disconnect();
            this._previewObserver = null;
        }

        preview.setAttribute('data-ypp-hero-projected', 'true');
        
        // Move the preview into our hero element, just like main 1 does!
        this._heroState.heroElement.appendChild(preview);
        
        // Apply inline styles to override any YouTube positioning
        preview.style.cssText = 'position: absolute !important; top: 0 !important; left: 0 !important; width: 100% !important; height: 100% !important; transform: none !important; pointer-events: none !important; opacity: 1 !important; z-index: 1 !important;';
        
        // Make the inner video element also span the whole container
        const fixVideo = () => {
