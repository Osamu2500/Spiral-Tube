...
function processDirectory(dir) {
    ...
    for (const file of files) {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
            ...
            // Clean global attachments
            content = content.replace(/window\.YPP\s*=\s*window\.YPP\s*\|\|\s*\{\};\n?/g, () => { modified = true; return ''; });
            content = content.replace(/window\.YPP\.features\s*=\s*window\.YPP\.features\s*\|\|\s*\{\};\n?/g, () => { modified = true; return ''; });
            
            // Format 1: class Name extends ... \n window.YPP.features.Name = Name;
            const regex1 = /class\s+([A-Za-z0-9_]+)([\s\S]*?\{)/g;
            let match1;
            let matchedClass = null;
            while ((match1 = regex1.exec(content)) !== null) {
                // Check if it's actually exported manually later
                const className = match1[1];
                if (content.includes(`window.YPP.features.${className} = ${className};`)) {
                    content = content.replace(match1[0], () => {
                        modified = true;
                        matchedClass = className;
                        const info = getFeatureInfo(className);
                        return `export class ${className}${match1[2]}
...
`;
                    ...
                    content = content.replace(new RegExp(`window\\.YPP\\.features\\.${className}\\s*=\\s*${className};\\n?`, 'g'), () => {
                        modified = true;
                        return '';
                    });
                }
            ...
            // Format 2: window.YPP.features.Name = class Name ...
            const regex2 = /window\.YPP\.features\.([a-zA-Z0-9_]+)\s*=\s*class\s+([a-zA-Z0-9_]+)([\s\S]*?\{)/g;
            content = content.replace(regex2, (match, attachName, className, rest) => {
                modified = true;
                const info = getFeatureInfo(className);
...
console.log('Feature files updated 2.');
